#Standard pipeline with some changes Josh suggested 
#Date: May 14, 2020

mySDM = '/mnt/research/ChomiukLab/LocalGroupX/NGC6822_v2.0/13A-213_05_15_2013_continuum.ms'
...
try:
    ...
    hifv_checkflag(checkflagmode='bpd') #changed as Josh suggested
    hifv_semiFinalBPdcals(weakbp=False)
    hifv_checkflag(checkflagmode='allcals')#changed as Josh suggested    hifv_checkflag(checkflagmode='semi')
    hifv_semiFinalBPdcals(weakbp=False)
    hifv_solint(pipelinemode="automatic")
    hifv_fluxboot2(fitorder=-1)
    hifv_finalcals(weakbp=False)
    hifv_flagcal(pipelinemode="automatic") #changed as Josh suggested
